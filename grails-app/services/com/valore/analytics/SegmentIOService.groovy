...
class SegmentIOService implements InitializingBean {
     ...
     *
     * @param anonId
     *            anonymous id the new user account is being linked to
     */
    void alias(def userId, def anonId) {
        if (enabled) {
            log.trace "Alias from userId=$userId to anonId=$anonId"
            analytics.alias(
                    userId.toString(),
                    anonId.toString()
            )
    ...
    void group(def userId, def groupId, Map traits = [:], Map options = [:]) {
        if (enabled) {
            log.trace "Group userId=$userId groupId=$groupId traits=$traits"
            analytics.group(
                    userId.toString(),
                    groupId.toString(),
    ...
    void identify(def userId, Map traits = [:], DateTime timestamp = null, Map options = [:]) {
        if (enabled) {
            log.trace "Identify userId=$userId traits=$traits timestamp=$timestamp options=$options"
            analytics.identify(
                    userId.toString(),
                    traits ? new Traits(*traits.collect { k, v -> [k, v] }.flatten()) : null,
    ...
    void page(def userId, String name, String category, Map properties = [:], DateTime timestamp = null, Map options = [:]) {
        if (enabled) {
            log.trace "Page userId=$userId name=$name category=$category properties=$properties timestamp=$timestamp options=$options"
            analytics.page(
                    userId.toString(),
                    name,
                    category,
    ...
    void screen(def userId, String name, String category, Map properties = [:], DateTime timestamp = null, Map options = [:]) {
        if (enabled) {
            log.trace "Screen userId=$userId name=$name category=$category properties=$properties timestamp=$timestamp options=$options"
            analytics.screen(
                    userId.toString(),
                    name,
                    category,
    ...
    void track(def userId, String event, Map properties = [:], DateTime timestamp = null, Map options = [:]) {
        ...
        Props props = properties ? new Props(*propify(properties)) : null

        if (enabled) {
            log.debug "Tracking userId=$userId event=$event properties=$properties timestamp=$timestamp options=$options"
            analytics.track( userId.toString(), event, props, buildOptions(options, timestamp) )
        }
    }

    // PRIVATE

    private static Collection propify( Map properties ) {
        ...
        properties.each { k, v ->
            ...
            if(v instanceof ArrayList){
                ...
                v.each { c.add(new HashMap<String, Object>(it)) }
                collection.add(c)
            } else if( v instanceof HashMap ){
                collection.add(propify(v))
            } else {
                collection.add(v)
            }
        }
    ...
    private static Options buildOptions(Map options, DateTime timestamp = null) {
        ...
        if (options.ip || options.language || options.user_agent || options.page || options.campaign || options["Google Analytics"] ) {
            ...
            if (options.user_agent) {
                sioContext.user_agent = options.user_agent
            }
            if (options.page) {
                sioContext.page = options.page
            }
            if (options.campaign) {
                sioContext.campaign = options.campaign
            }
            if (options["Google Analytics"]) {
                sioContext["Google Analytics"] = options["Google Analytics"] as HashMap<String, String>
            }
    ...
    private def getConfig() {
        grailsApplication.config.segmentio
    }

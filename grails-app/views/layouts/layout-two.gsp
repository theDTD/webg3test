...
<body class="${pageProperty( name:'body.class' )}" itemscope="itemscope" itemtype="http://schema.org/WebPage">
  ...
  <g:if test="${controllerName == 'cart'}">
    <g:render template="/foot-includes/google-adwords-params" model='[productList: cart?.items?.collect({it.product.id}), pageType : "cart", amount : "${itemTotal / 100}"]'/>
  </g:if>
